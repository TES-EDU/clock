...
import { getStudentMathResults, deleteStudentMathResults, type MathResultRow } from '../../../lib/supabase';

const scoreColor = (score: number) => score >= 80 ? 'text-sb-correct-dark' : score >= 60 ? 'text-sb-orange-dark' : 'text-sb-wrong-dark';
const groupAccent = (_unitId: string) => ({ chip: 'bg-sb-primary-pale text-sb-primary-dark', dot: 'bg-sb-primary' });

const formatEncodedTime = (val: number | null, unitTitle: string) => {
  if (val === null || val === undefined) return '';
  const levelStr = unitTitle.replace('CLOCK_L', '');
...
function TrendChart({ data, width = 680, height = 160 }: { data: TrendPoint[]; width?: number; height?: number }) {
  ...
  return (
    <svg width={width} height={height} className="block w-full">
      ...
      {data.map((d, i) => (
        <circle key={i} cx={xs[i]} cy={ys[i]} r="4.5" fill="white" stroke={groupAccent(d.level).dot} strokeWidth="2.5" />
      ))}
      {data.map((d, i) => {
...
function LevelBars({ rows }: { rows: MathResultRow[] }) {
  const byLevel = useMemo(() => {
    ...
    const levels = ['CLOCK_L1', 'CLOCK_L2', 'CLOCK_L3', 'CLOCK_L4', 'CLOCK_L5', 'CLOCK_L6', 'CLOCK_L7', 'CLOCK_L8'] as const;
    return levels
      .filter(k => map[k])
      .map(k => ({ level: k.replace('CLOCK_L', '레벨 '), avg: Math.round(map[k].sum / map[k].total), count: map[k].total }));
  }, [rows]);
    ...
    <div className="space-y-3">
      {byLevel.map(({ level, avg, count }) => (
        <div key={level} className="flex items-center gap-3">
          <div className="w-14 text-xs font-bold text-sb-muted text-right shrink-0">{level}</div>
...
export default function StudentHistoryScreen({ studentName, onBack, onReportClick }: Props) {
  const [rows, setRows] = useState<MathResultRow[]>([]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    setDeleting(true);
    const ok = await deleteStudentMathResults(studentName);
    setDeleting(false);
    if (ok) onBack();
  }, [studentName, onBack]);
  ...
  const load = async () => {
    setLoading(true);
    setRows(await getStudentMathResults(studentName));
    setLoading(false);
  };
  ...
  const stats = useMemo(() => {
    ...
    const scores = rows.map(r => r.accuracy);
  ...
  const frequentWrong = useMemo(() => {
    const map = new Map<string, { expression: string; correctAnswer: number; wrongCount: number; totalSeen: number; unitId: string }>();
    rows.forEach(r => {
      (r.incorrect_answers ?? []).forEach(w => {
        const k = w.expression;
        const e = map.get(k) ?? { expression: w.expression, correctAnswer: w.correctAnswer, wrongCount: 0, totalSeen: 0, unitId: w.unitId };
        e.wrongCount += 1; e.totalSeen += 1;
        map.set(k, e);
      });
      (r.correct_answers ?? []).forEach(c => {
        const k = c.expression;
        const e = map.get(k);
    ...
    <div className="bg-sb-surface border border-sb-line rounded-2xl p-5">
      <div className="flex items-center gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13px] text-sb-muted mt-0.5">TES 시계 응시자</div>
        </div>
        <button
  ...
  return (
    <div className="min-h-screen bg-sb-bg flex flex-col">
      <header className="bg-sb-surface border-b border-sb-line h-14 px-5 flex items-center gap-2.5 shrink-0">
        ...
        <span className="text-sm font-extrabold text-sb-ink -tracking-tight">TES 시계</span>
        ...
        <div className="space-y-3.5">
          ...
          {frequentWrong.length > 0 && (
            <div className="bg-sb-surface border border-sb-line rounded-2xl p-[18px]">
              <div className="flex items-center gap-1.5 mb-3">
                ...
                <div className="text-sm font-extrabold text-sb-ink">자주 틀리는 시간</div>
                ...
                {frequentWrong.map((w, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2.5 px-3 py-2.5 bg-sb-bg rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-bold text-sb-ink">{w.expression}</div>
                        <div className="text-[11px] text-sb-muted">{formatEncodedTime(w.correctAnswer, w.unitId)}</div>
                      </div>
          ...
          <div className="bg-sb-surface border border-sb-line rounded-2xl overflow-hidden">
            ...
            {loading ? (
              ...
              const c = groupAccent(h.unit_title);
              ...
              return (
                ...
                >
                  <div className={`text-[22px] font-extrabold w-[54px] text-right -tracking-[0.02em] tabular-nums shrink-0 ${"text-sb-primary-dark"}`}>
                    {h.score}<span className="text-[11px]">%</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded tracking-wide ${c.chip}`}>{h.unit_title.startsWith('CLOCK_L') ? h.unit_title.replace('CLOCK_L', '레벨 ') : h.unit_title}</span>
                    ...
                    <div className="text-[11px] text-sb-muted flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-0.5"><Clock size={10} /> {formatDuration(h.time_seconds ?? 0)}</span>
        ...
        <div className="mx-auto max-w-[1280px] px-10 py-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-sb-muted mt-0.5">
                  {rows[0] ? `최근 응시 ${new Date(rows[0].created_at ?? '').toLocaleDateString('ko-KR')}` : 'TES 시계 응시자'}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-0 grid grid-cols-12 gap-6">
            ...
            <section className="col-span-4 rounded-2xl bg-sb-surface border border-sb-line p-5 lg:p-6">
              <div className="text-xs font-extrabold tracking-[0.22em] text-sb-primary-dark mb-4">자주 틀리는 시간</div>
              {frequentWrong.length > 0 ? (
                <div className="space-y-2">
                  {frequentWrong.map((w, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-sb-wrong-pale/40">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-sb-ink truncate">{w.expression}</div>
                        <div className="text-xs text-sb-muted truncate">{formatEncodedTime(w.correctAnswer, w.unitId)}</div>
                      </div>
              ...
              {loading ? (
                ...
                <div className="divide-y divide-sb-line-soft">
                  {rows.map(r => {
                    ...
                    const c = groupAccent(r.unit_title);
                    return (
                      <button
                        key={r.id}
                      ...
                      >
                        <div className={`text-[22px] font-extrabold w-[54px] text-right tabular-nums shrink-0 ${scoreColor(r.accuracy)}`}>
                          {r.accuracy}<span className="text-[11px]">%</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded ${c.chip}`}>{r.unit_title.startsWith('CLOCK_L') ? r.unit_title.replace('CLOCK_L', '레벨 ') : r.unit_title}</span>
